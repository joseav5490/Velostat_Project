from long_velostat_test import shift_0, shift_1, shift_2, shift_3
import time
...
def state_output(state, voltage, pin_num, timer_B, key, trigval1, trigval2, trigval3, LED):
    if(state[key] < 3.3):
        GPIO.output(pin_num[key], GPIO.LOW)
        LED[key] = False
        variable_change(timer_B, key, time.time())
    elif (state[key] > trigval3[key] and voltage[key] > .2 and voltage[key] < 1):
        GPIO.output(pin_num[key], GPIO.HIGH)
        LED[key] = True
    elif (state[key] > trigval2[key] and voltage[key] > 1 and voltage[key] < 3.6):
        GPIO.output(pin_num[key], GPIO.HIGH)
        LED[key] = True
    elif (state[key] > trigval1[key] and voltage[key] > 3.6):
        GPIO.output(pin_num[key], GPIO.HIGH)
        LED[key] = True
    return (LED[key])        
...
# Get initial value during startup.
...
while True:
    ...
    LED[0] = state_output(state, voltage, pin_number, timer_b, 0, triggerval1, triggerval2, triggerval3, LED)

    shift_1()
    state[1], sum_avg[1], voltage[1] = check_sums(state, sum_avg, chan1.voltage, timer_a, 1)
    LED[1] = state_output(state, voltage, pin_number, timer_b, 1, triggerval1, triggerval2, triggerval3, LED)

    shift_2()
    state[2], sum_avg[2], voltage[2] = check_sums(state, sum_avg, chan2.voltage, timer_a, 2)
    LED[2] = state_output(state, voltage, pin_number, timer_b, 2, triggerval1, triggerval2, triggerval3, LED)

    shift_3()
    state[3], sum_avg[3], voltage[3] = check_sums(state, sum_avg, chan3.voltage, timer_a, 3)
    LED[3] = state_output(state, voltage, pin_number, timer_b, 3, triggerval1, triggerval2, triggerval3, LED)

    #for i in range(0, amount_of_sensors):
    #    LED[i] = state_output(state, voltage, pin_number, timer_b, i, triggerval1, triggerval2, triggerval3, LED)
